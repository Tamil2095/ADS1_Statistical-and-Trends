# -*- coding: utf-8 -*-
"""
Created on Tue Dec 12 13:03:18 2023
...
the X and Y values are same Indicator name'''
    ...
    sns.heatmap(numeric_df.corr(), cmap='RdYlGn', square=True,
...
countries for the given years, then applyig the dataframe into the function, 
labelling the X-axis and Y-axis, Naming the barplot, Create legend in
best place'''

    sns.set_style('whitegrid')
    df_filtered = df[df['Years'].isin([2001, 2005, 2010, 2015, 2020])]
    df_filtered.plot(x=x_value, y=y_value, kind='bar', title=head_title,
                     color=colors, width=0.65, figsize=(5, 3),
...
title for the histogram is also mentioned'''
    ...
    plt.figure()
    plt.hist(data, bins=20, alpha=0.6, color='green', edgecolor='black')
    plt.grid(True)
    title_components = [
...
filename = r'C:/Users/TAMILSELVAN/API_19_DS2_en_csv_v2_5998250.csv'
...
create_barplot(frwt_t, 'Years', ['China', 'United Kingdom', 'Indonesia',
                                 'India', 'Pakistan', 'United States'],
